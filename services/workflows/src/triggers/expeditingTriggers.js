...
// ============================================================================
// TRIGGER 6: PII Case Closed
// When a PII case is closed/resolved, auto-set piiComplete='yes' on opportunity
// ============================================================================

/**
 * Trigger: PII Case Closed
 * When a PII-type case is closed, automatically set piiComplete='yes'
 * on the related opportunity
 */
export async function onPiiCaseClosed(caseId, userId) {
  console.log(`[Expediting Trigger] PII Case Closed: ${caseId}`);
  ...
  try {
        ...
        account: {
          include: {
            opportunities: {
              where: {
                status: { not: 'CLOSED_LOST' },
                piiComplete: 'no', // Only update if piiComplete is 'no'
              },
              orderBy: { createdAt: 'desc' },
              take: 1,
            },
    ...
    // Check if this is a PII-related case
    const isPiiCase =
      closedCase.type === 'PII' ||
      closedCase.type?.toLowerCase().includes('pii') ||
      closedCase.subject?.toLowerCase().includes('pii') ||
      closedCase.description?.toLowerCase().includes('pii');

    if (!isPiiCase) {
      console.log(`[Expediting Trigger] Case ${caseId} is not PII-related, skipping`);
      return result;
    }
    ...
    if (!opportunity) {
      console.log(`[Expediting Trigger] No eligible opportunity found for PII Complete update`);
      return result;
    }

    // Update the opportunity's piiComplete field
    await prisma.opportunity.update({
      where: { id: opportunity.id },
      data: {
        piiComplete: 'yes',
    ...
    console.log(`[Expediting Trigger] Set piiComplete='yes' for opportunity ${opportunity.id}`);

    // Create activity log
    await prisma.activity.create({
      data: {
        type: 'FIELD_CHANGE',
        subject: 'PII Complete (Auto-Updated)',
        body: `PII Complete automatically set to 'Yes' when PII case ${closedCase.caseNumber} was closed.`,
        status: 'COMPLETED',
        opportunityId: opportunity.id,
        accountId: closedCase.accountId,
        userId: userId,
    ...
    console.error('[Expediting Trigger] onPiiCaseClosed failed:', error);
    result.error = error.message;
    return result;
  }
}
...
// ============================================================================
// TRIGGER 7: Permit Required
// When permitRequired is set to true, create a permit case
// ============================================================================

/**
 * Trigger: Permit Required
 * When permitRequired is set to true, create a permit case
 * Note: The primary case creation can be handled via the frontend modal,
 * but this provides an automated case creation option
 */
export async function onPermitRequired(opportunityId, userId, autoCreateCase = true) {
  console.log(`[Expediting Trigger] Permit Required for Opportunity: ${opportunityId}`);
  ...
  if (!autoCreateCase) {
    // Case creation is handled by frontend modal - just log
    console.log(`[Expediting Trigger] Permit Required detected - case creation delegated to user`);
    return result;
  }

  try {
    // Update permit status to pending
    await prisma.opportunity.update({
      where: { id: opportunityId },
      data: {
    ...
    const caseResult = await createExpeditingCase(
      opportunityId,
      CASE_SUBJECTS.PERMIT,
      'This job requires a permit before installation can proceed. Please submit the permit application and track the approval process.',
      CASE_TYPES.PERMIT,
      null, // No auto-assignment for permits - user selects in modal
      userId
    );
    ...
    await prisma.activity.create({
      data: {
        type: 'FIELD_CHANGE',
        subject: 'Permit Required',
        body: `Permit marked as required. ${caseResult.existing ? 'Existing case found.' : `Case ${caseResult.case?.caseNumber} created.`}`,
        status: 'COMPLETED',
        opportunityId: opportunityId,
        userId: userId,
        occurredAt: new Date(),
    ...
    console.error('[Expediting Trigger] onPermitRequired failed:', error);
    result.error = error.message;
    return result;
  }
}
...
// ============================================================================
// TRIGGER 8: Permit Case Closed
// When a permit case is closed/resolved, auto-set permit fields on opportunity
// ============================================================================

/**
 * Trigger: Permit Case Closed
 * When a Permit-type case is closed, automatically set:
 * - permitObtained = true
 * - permitStatus = 'approved'
 * - permitApprovedDate = now (if not already set)
 */
export async function onPermitCaseClosed(caseId, userId) {
  console.log(`[Expediting Trigger] Permit Case Closed: ${caseId}`);
  ...
  try {
    ...
    // Check if this is a Permit-related case
    const isPermitCase =
      closedCase.type === CASE_TYPES.PERMIT ||
      closedCase.type?.toLowerCase().includes('permit') ||
      closedCase.subject?.toLowerCase().includes('permit');

    if (!isPermitCase) {
      console.log(`[Expediting Trigger] Case ${caseId} is not Permit-related, skipping`);
      return result;
    }
    ...
    if (!opportunity) {
      console.log(`[Expediting Trigger] No eligible opportunity found for Permit update`);
      return result;
    }

    // Update the opportunity's permit fields
    const updateData = {
      permitObtained: true,
      permitStatus: 'approved',
    };

    // Only set approved date if not already set
    const existingOpp = await prisma.opportunity.findUnique({
      where: { id: opportunity.id },
      select: { permitApprovedDate: true },
    });

    if (!existingOpp?.permitApprovedDate) {
      updateData.permitApprovedDate = new Date();
    }
    ...
    console.log(`[Expediting Trigger] Set permit fields for opportunity ${opportunity.id}`);

    // Create activity log
    await prisma.activity.create({
      data: {
        type: 'FIELD_CHANGE',
        subject: 'Permit Approved (Auto-Updated)',
        body: `Permit automatically set to Approved when Permit case ${closedCase.caseNumber} was closed.`,
        status: 'COMPLETED',
        opportunityId: opportunity.id,
        accountId: closedCase.accountId,
        userId: userId,
    ...
    console.error('[Expediting Trigger] onPermitCaseClosed failed:', error);
    result.error = error.message;
    return result;
  }
}
...
export async function evaluateExpeditingTriggers(opportunityId, changes, previousValues, userId) {
  ...
  // Check Permit Required trigger
  if (changes.permitRequired === true && previousValues?.permitRequired !== true) {
    results.permitRequired = await onPermitRequired(opportunityId, userId, true);
  }

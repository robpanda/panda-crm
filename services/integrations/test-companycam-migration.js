...
async function getPhotoUrlsFromApi(photoId, token) {
  ...
  console.log(`  Fetching photo details from CompanyCam API for photo: ${photoId}`);
  ...
  // Extract the actual downloadable URLs from the API response
  ...
  console.log(`  Got original URL: ${originalUrl?.substring(0, 80)}...`);
...
async function createPhotoRecord(photo, project, s3Key, s3ThumbnailKey) {
  // Find the PhotoProject record via opportunityId
  // PhotoProject doesn't have companyCamId - it's linked via Opportunity
  const photoProject = await prisma.photoProject.findFirst({
    where: { opportunityId: project.opportunityId }
  });

  if (!photoProject) {
    console.log(`  ⚠ No PhotoProject found for Opportunity ID ${project.opportunityId}`);
    return null;
  }
...
async function runMigrationTest() {
  ...
  try {
    ...
    for (const photo of photos) {
      ...
      console.log(`  Stored URL (unusable): ${photo.photoUrl}`);

      try {
        // Fetch the actual downloadable URLs from CompanyCam API
        // (stored photoUrl returns HTML login page, need uris.original from API)
        ...
        // Download and upload main photo using API-provided URL
        ...
        // Download and upload thumbnail if available from API

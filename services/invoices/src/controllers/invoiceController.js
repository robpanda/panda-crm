...
async function generateInvoicePdfInternal(invoice) {
  ...
  const invoiceAdjustments = Array.isArray(invoice.additionalCharges) ? invoice.additionalCharges : [];
  const supplementsTotal = invoiceAdjustments
    .filter((charge) => Number(charge.amount ?? charge.fixedAmount ?? 0) > 0)
    .reduce((sum, charge) => sum + Number(charge.amount ?? charge.fixedAmount ?? 0), 0);
  ...
  if (invoiceAdjustments.length > 0) {
    y -= 28;
    page.drawText('Supplements & Discounts', { x: tableLeft, y, size: 11, font: fontBold, color: COLORS.dark });
    y -= 18;

    for (const charge of invoiceAdjustments) {
      const chargeAmount = Number(charge.amount ?? charge.fixedAmount ?? 0);
      const chargeLabel = chargeAmount < 0 ? 'Discount' : 'Supplement';
      const chargeName = charge.name || chargeLabel;
      page.drawText(`${chargeLabel}: ${chargeName}`, { x: tableLeft + 10, y, size: 10, font: fontRegular, color: COLORS.dark });
  ...
  if (supplementsTotal > 0) {
    y -= 18;
    page.drawText('Supplements:', { x: totalsX, y, size: 10, font: fontRegular, color: COLORS.gray });
    page.drawText(formatCurrency(supplementsTotal), { x: width - 80, y, size: 10, font: fontRegular, color: COLORS.dark });
  }

  if (discountsTotal > 0) {
    y -= 18;
    page.drawText('Discounts:', { x: totalsX, y, size: 10, font: fontRegular, color: COLORS.gray });
    page.drawText(`-${formatCurrency(discountsTotal)}`, { x: width - 80, y, size: 10, font: fontRegular, color: COLORS.dark });
  }

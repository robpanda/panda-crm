...
class LeadService {
  ...
  async getLeadCounts(currentUserId, ownerId = null, ownerIds = []) {
    ...
    const knownStatuses = ['NEW', 'CONTACTED', 'QUALIFIED', 'UNQUALIFIED', 'NURTURING', 'CONVERTED', 'SCHEDULED'];
    ...
    const [total, mine, ...statusBuckets] = await Promise.all([
      ...
      resolvedUserId
        ...
        : Promise.resolve(0),
      ...knownStatuses.map((status) => prisma.lead.count({ where: { ...baseWhere, status } })),
    ]);
    ...
    knownStatuses.forEach((status, index) => {
      const value = statusBuckets[index] || 0;
      counts[status] = value;
      counts[status.toLowerCase()] = value;
    });
  ...
  createLeadWrapper(lead) {
    const wrapper = {
      ...
      leadRank: lead.leadRank ?? lead.lead_rank,
      scoreFactors: lead.scoreFactors ?? lead.score_factors,
      scoredAt: lead.scoredAt ?? lead.scored_at,
      ...
      medianAge: lead.medianAge ?? lead.median_age,
      enrichedAt: lead.enrichedAt ?? lead.enriched_at,

...
export function validateAppointmentResultPayload(payload = {}) {
  ...
  if (followUpType === FOLLOW_UP_TYPES.VIRTUAL) {
    ...
    if (!virtualTask.taskType) {
      errors.push({ field: 'virtualTask.taskType', message: 'Virtual follow-up task type is required' });
    }
    if (!virtualTask.dueDate) {
      errors.push({ field: 'virtualTask.dueDate', message: 'Virtual follow-up due date is required' });
    }
    if (virtualTask.dueDate && !parseDateTimeParts(virtualTask.dueDate, virtualTask.dueTime)) {
      errors.push({ field: 'virtualTask.dueDate', message: 'Virtual follow-up due date/time is invalid' });
    }
  }
  ...
  if (followUpType === FOLLOW_UP_TYPES.IN_PERSON) {
    ...
    if (!inPerson.date) {
      errors.push({ field: 'inPersonAppointment.date', message: 'In-person appointment date is required' });
    }
    if (!inPerson.time) {
      errors.push({ field: 'inPersonAppointment.time', message: 'In-person appointment time is required' });
    }
    if (inPerson.date && inPerson.time && !parseDateTimeParts(inPerson.date, inPerson.time)) {
      errors.push({ field: 'inPersonAppointment.time', message: 'In-person appointment date/time is invalid' });
    }
  }

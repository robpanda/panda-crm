...
function mapCommissionType(sfType) {
  const typeMap = {
    ...
    'Regional Override': 'REGIONAL_OVERRIDE',
  ...
  return typeMap[sfType] || 'BACK_END_COMMISSION';
}
...
function transformCommission(sfCommission, accountIdMap, contractIdMap, invoiceIdMap, userIdMap) {
  ...
  return {
    ...
    commissionType: mapCommissionType(sfCommission.Commission_Type__c),
    status: mapCommissionStatus(sfCommission.Status__c),
    statusCode: sfCommission.Status_Code__c || null,
    ...
    invoiceId: sfCommission.Invoice__c ? invoiceIdMap.get(sfCommission.Invoice__c) || null : null,
    accountId: sfCommission.Customer_Name__c ? accountIdMap.get(sfCommission.Customer_Name__c) || null : null,
    ownerId: commissionOwnerId ? userIdMap.get(commissionOwnerId) || null : null,

    // Store Salesforce ID for owner if not resolved
    ownerSalesforceId: commissionOwnerId || null,

    // Financial fields
    commissionValue: sfCommission.Commission_Value__c ? parseFloat(sfCommission.Commission_Value__c) : null,
    contractValue: sfCommission.Contract_Value__c ? parseFloat(sfCommission.Contract_Value__c) : null,
    commissionRate: sfCommission.Commission_Rate__c ? parseFloat(sfCommission.Commission_Rate__c) : null,
    commissionRateOfPay: sfCommission.Commission_Rate_of_Pay__c ? parseFloat(sfCommission.Commission_Rate_of_Pay__c) : null,
    invoiceTotal: sfCommission.Invoice_Total__c ? parseFloat(sfCommission.Invoice_Total__c) : null,
    paidAmount: sfCommission.Paid_Amount__c ? parseFloat(sfCommission.Paid_Amount__c) : null,
    // Use RequestedAmount__c (newer) or Requested_Amount__c (older)
    ...
    collectedPercent: sfCommission.Collected__c ? parseFloat(sfCommission.Collected__c) : null,
    payrollDifference: sfCommission.Payroll_Difference__c ? parseFloat(sfCommission.Payroll_Difference__c) : null,

    // Status flags
    commissionPaid: sfCommission.Commission_Paid__c || false,
    preCommissionPaid: sfCommission.Pre_Commission_Paid__c || false,
    keepTheCashPromo: sfCommission.Keep_the_Cash_Promo__c || false,

    // Dates
    payrollUpdateDate: sfCommission.Payroll_Update_Date__c ? new Date(sfCommission.Payroll_Update_Date__c) : null,
...
async function migrateCommissions(options = {}) {
  ...
  try {
    ...
    const commissions = await queryAllRecords(conn, query);
    console.log(`Found ${commissions.length} Commission records`);

    // Transform records
    console.log('Transforming records...');
    const transformedRecords = commissions.map(comm =>

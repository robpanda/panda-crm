...
export default function ResultAppointmentWizard({
      ...
      inPersonAppointment:
        state.followUpMode === 'IN_PERSON'
          ? {
              ...
              durationMinutes: Number(state.inPersonDurationMinutes || DEFAULT_FOLLOW_UP_DURATION_MINUTES),
  ...
  const renderInspectionNotCompletedStep = () => (
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Reason inspection did not occur</label>
        <select
  ...
  const renderNoClaimReasonStep = () => (
    <div className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Reason no claim was filed</label>
        <select
  ...
  const renderStepBody = () => {
    ...
    if (state.step === STEPS.INSURANCE_CLAIM) {
      return (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => dispatch({ type: ACTIONS.SELECT_INSURANCE_OUTCOME, value: 'filed-claim' })}
            className="border border-gray-200 rounded-lg p-4 text-left hover:border-green-500 hover:bg-green-50 transition"
          >
          ...
          <button
            type="button"
            onClick={() => dispatch({ type: ACTIONS.SELECT_INSURANCE_OUTCOME, value: 'no-claim-filed' })}
            className="border border-gray-200 rounded-lg p-4 text-left hover:border-red-500 hover:bg-red-50 transition"
          >
    ...
    if (state.step === STEPS.NO_PITCH_REASON) {
      return (
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Why not pitching retail?</label>
            <select
          ...
          <button
            type="button"
            onClick={() => goToStep(STEPS.CONFIRM)}
            disabled={!state.noPitchRetailReason || state.noPitchRetailReason === 'FOLLOW_UP_SCHEDULED'}
            className="w-full px-4 py-3 rounded-lg bg-green-600 text-white hover:bg-green-700 disabled:opacity-50"
          >
            Continue without follow-up
          </button>
      ...
      return (
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Claim filed date</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Date of loss (optional)</label>
              <input
                type="date"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Damage location (optional)</label>
            <input
              type="text"
    ...
    if (state.step === STEPS.RETAIL_NOT_SOLD_REASON) {
      return (
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Reason it did not sell</label>
            <select
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-2 text-sm">
            ...
            {disposition.followUpDate && (
              <div className="flex items-center justify-between gap-4">
                ...
                <span className="font-medium text-right">{formatDateTime(disposition.followUpDate)}</span>
              </div>
            )}
            {state.followUpMode && (
              <div className="flex items-center justify-between gap-4">

...
import { Eye, EyeOff, UserPlus, AlertCircle, Trophy, CheckCircle, Loader2 } from 'lucide-react';
import { championsApi } from '../services/api';

export default function ChampionJoin() {
  ...
  const loadInviteDetails = async () => {
    try {
      const data = await championsApi.getInviteDetails(token);
      setInvite(data);
      setFormData(prev => ({
        ...prev,
      ...
      if (errCode === 'NOT_FOUND') {
        setInviteError('This invite link is invalid. Please request a new invite.');
        ...
        setInviteError('This invite has already been used. If you already have an account, please sign in.');
      } else {
        setInviteError(err.response?.data?.error?.message || 'Failed to load invite details');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await championsApi.completeInvite(token, data);
      setSuccess(true);
    } catch (err) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 via-orange-50 to-amber-100 px-4">
        <div className="w-full max-w-md p-8 bg-white rounded-2xl shadow-xl text-center">
          ...
          <div className="space-y-3">
            <Link
              to="/champion/login"
              className="block w-full px-4 py-3 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors font-medium"
            >
              Sign In
            </Link>
            <Link
              to="/champion/register"
        ...
        <div className="w-full max-w-md p-8 bg-white rounded-2xl shadow-xl">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-6">
              Your Champion account is now active. Start earning rewards by referring customers to Panda Exteriors!
            </p>
            <p className="text-sm text-gray-500 mb-6">
              Download our mobile app to submit referrals and track your earnings on the go.
            </p>
            <div className="space-y-3">
              <a
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            You've been invited to become a Panda Champion!
          </p>
          {invite?.email && (
